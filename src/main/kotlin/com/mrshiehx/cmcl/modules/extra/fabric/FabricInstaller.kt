...
object FabricInstaller : ExtraInstaller() {
    override val extraName = "Fabric"
    override val extraMerger = FabricMerger
    ...
    override fun checkInstallable(gameJSON: JSONObject): Boolean {
        ...
        if (!isEmpty(VersionModuleUtils.getForgeVersion(gameJSON))) {
            println(CMCL.getString("INSTALL_MODLOADER_ALREADY_INSTALL_ANOTHER_ONE", extraName, "Forge"))
            return false
        }
        if (!isEmpty(VersionModuleUtils.getLiteloaderVersion(gameJSON))) {
            println(CMCL.getString("INSTALL_MODLOADER_ALREADY_INSTALL_ANOTHER_ONE", extraName, "LiteLoader"))
            return false
        }
        if (!isEmpty(VersionModuleUtils.getOptifineVersion(gameJSON))) {
            println(CMCL.getString("INSTALL_MODLOADER_ALREADY_INSTALL_ANOTHER_ONE", extraName, "OptiFine"))
            return false
        }
