...
enum class OperatingSystem(val checkedName: String) {
    ...
    companion object {
         ...
         */
        val CURRENT_OS: OperatingSystem
        /**
        ...
        SUGGESTED_MEMORY = (int) (Math.round(1.0 * TOTAL_MEMORY / 4.0 / 128.0) * 128);*/
            var nativeCharset = Charset.defaultCharset()
            val nativeEncoding = System.getProperty("native.encoding")
            try {
                if (nativeEncoding != null && !nativeEncoding.equals(nativeCharset.name(), ignoreCase = true)) {
                ...
                if (nativeCharset === StandardCharsets.UTF_8 || nativeCharset === StandardCharsets.US_ASCII) {
                    ...
                    "GBK".equals(nativeCharset.name(), ignoreCase = true) ||
                    "GB2312".equals(nativeCharset.name(), ignoreCase = true)
                ) {
