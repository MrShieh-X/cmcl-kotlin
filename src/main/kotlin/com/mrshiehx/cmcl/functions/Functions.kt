...
object Functions {
    operator fun get(name: String): Function? = when (name) {
        "install" -> InstallFunction
        "version" -> VersionFunction
        "account" -> AccountFunction
        "config" -> ConfigFunction
        "simplify" -> SimplifyCommandFunction
        "jvmArgs" -> JVMArgsFunction
        "gameArgs" -> GameArgsFunction
        "mod" -> ModFunction
        "modpack" -> ModpackFunction
        else -> null
    }
}
